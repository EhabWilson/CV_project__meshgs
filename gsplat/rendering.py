...
def rasterization_2dgs(
    ...
    if distributed:
        if packed:
            ...
            (means2d, depths, ray_transformations, normals, opacities, densify, colors) = all_to_all_tensor_list(
                world_size,
                [means2d, depths, ray_transformations, normals, opacities, densify, colors],
                cnts,
                output_splits=collected_splits,
            )
            ...
            means2d = reshape_view(C, means2d, N_world)
            depths = reshape_view(C, depths, N_world)
            ray_transformations = reshape_view(C, ray_transformations, N_world)
            normals = reshape_view(C, normals, N_world)
            opacities = reshape_view(C, opacities, N_world)
            densify = reshape_view(C, densify, N_world)
            colors = reshape_view(C, colors, N_world)
    ...
    if render_mode in ["RGB+ED", "RGB+D"]:
        ...
        # render_depths = depth_for_normal
...
def rasterization_2dgs_inria_wrapper(
    ...
    **kwargs,
    ...
    render_depth_expected = render_depth_expected / render_alphas.clamp(min=1e-10)
    ...
    return (render_colors, render_depth, render_alphas), meta
