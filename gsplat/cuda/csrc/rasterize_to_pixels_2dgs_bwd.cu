    ...
    if (n_isects) {
        ...
        rasterize_to_pixels_bwd_2dgs_kernel<CDIM, float>
            <<<blocks, threads, shared_mem, stream>>>(
                ...
                render_colors.data_ptr<float>(), render_depths.data_ptr<float>(), render_alphas.data_ptr<float>(), 
                last_ids.data_ptr<int32_t>(), median_ids.data_ptr<int32_t>(),
                v_render_colors.data_ptr<float>(), v_render_depths.data_ptr<float>(), v_render_alphas.data_ptr<float>(),
...
#define __GS__CALL_(N)                                                                 \
    case N:                                                                            \
        return call_kernel_with_dim<N>(                                                \
            ...
            render_colors, render_depths, render_alphas, last_ids, median_ids,         \
            v_render_colors, v_render_depths, v_render_alphas, v_render_normals,        \
