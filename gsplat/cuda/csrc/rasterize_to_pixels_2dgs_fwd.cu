...
call_kernel_with_dim(
    ...
    rasterize_to_pixels_fwd_2dgs_kernel<CDIM, float>
        <<<blocks, threads, shared_mem, stream>>>(
            ...
            renders.data_ptr<float>(),depths.data_ptr<float>(), alphas.data_ptr<float>(),
    ...
    return std::make_tuple(renders, depths, alphas, render_normals, render_distort, render_median, last_ids, median_ids);
}
